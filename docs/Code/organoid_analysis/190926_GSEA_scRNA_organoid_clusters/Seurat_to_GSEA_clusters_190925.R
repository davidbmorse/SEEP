#GSEA organoid CLUSTERS (to estimate positional differences in cell states) 9/26/19
...
Organoid <- readRDS("/Users/morsedb/Documents/Projects/PSS paint sort sequence/Analysis/organoidsComb_190226/R_scripts/organoids_comb_v3.rds")

#set identity to clusters
Organoid <- SetIdent(Organoid, value = "res.0.6")

# add 'ORG' ident - 'ORG' simply denotes 'all cells ---------
x = Organoid
x = SetIdent(x, cells = NULL, value = rep("ORG",nrow(x@meta.data)))
inf = x@meta.data
nam=sapply(strsplit(rownames(inf), "_"), function(x) paste(x[1],x[2], sep="_"))
nam=paste("ORG", nam,1:nrow(inf), sep="_")
x <- RenameCells(x, add.cell.id = nam)
mergedData <- merge(Organoid, x, project="ORG")
mergedData$new.ident <- Idents(mergedData)

#save(mergedData, file = "/Users/morsedb/Documents/Projects/PSS paint sort sequence/Analysis/organoidsComb_190226/R_scripts/190926_GSEA_scRNA_organoid_clusters/data/mergedData.rda")
...
sel <- rowSums(means) > 0
means <- means[sel,]
difs = as.data.frame(means - means[,'ORG'])

# 4 save file as 'preranked' rda file
# save to rda file
save(difs, means, file = "data/PrerankOrg.rda")
...
subset.matrix <- Organoid@assays$RNA@data[genes.use, ]
Organoid_GSEA <- CreateSeuratObject(subset.matrix)
old_matrix_idents <- Organoid@meta.data[, "res.0.6"]
Organoid_GSEA <- AddMetaData(object = Organoid_GSEA, metadata = old_matrix_idents, col.name = "res.0.6")
Idents(object = Organoid_GSEA) <- "res.0.6"
#and save it for later import into GSEA
save(Organoid_GSEA, file = "data/scaledClusterdDataGSEA.rda")
...
gmtlist(path.gmtFolder="//Users/morsedb/Documents/Projects/PSS paint sort sequence/Analysis/organoidsComb_190226/R_scripts/190926_GSEA_scRNA_organoid_clusters/MSigDB/MSigDB_subsets",
