...
getGSEA <- function(pathway, collection = 'h.all', gsea, layer, fdr=0.1, out=c('LE','stat')) 
{
...
getHeatOrder = function(dx, genes, threshold=0.35){
  ...
  mat = calc.rankMetric(dx@data[genes,], dx@ident, out='difs')
...
getHeat= function(dx, genes, ord, layer, threshold=4){
  ...
  mat = t(apply(dx@data[genes, dx@ident==layer],1, function(x) x - calc.logMean(x)))
  scale.range= c(-threshold, threshold)
  zak= seq(scale.range[1],scale.range[2],0.01)
  nzak=length(zak)
  pal = rev(brewer.pal(11,'Spectral'))
