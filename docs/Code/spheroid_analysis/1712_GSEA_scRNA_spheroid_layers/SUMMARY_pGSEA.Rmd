...
knitr::opts_chunk$set(root.dir="/Users/morsedb/Documents/Projects/PSS paint sort sequence/Analysis/PEO1_spheroids/1712_GSEA_scRNA_spheroid_layers") 
...
loc="INNER"
fid=which(gsea_INNER$h.all$pathway==pathway)
gseaRes=gsea_INNER$h.all[fid, ]
nameGset = gseaRes$pathway
collection="h.all"
gset = L_subsets[[collection]][[nameGset]]
ranks = difs$INNER
names(ranks) = rownames(difs)
I_alpha = sort(unlist(gseaRes$leadingEdge))
plot.ES(rank.statistic = ranks, gsea=gseaRes, gene.set=gset, name=nameGset, pathwaySource=collection, location=loc)

loc="OUTER"
fid=which(gsea_OUTER$h.all$pathway==pathway)
gseaRes=gsea_OUTER$h.all[fid, ]
nameGset = gseaRes$pathway
collection="h.all"
gset = L_subsets[[collection]][[nameGset]]
ranks = difs$OUTER
names(ranks) = rownames(difs)
O_alpha = sort(unlist(gseaRes$leadingEdge))
plot.ES(rank.statistic = ranks, gsea=gseaRes, gene.set=gset, name=nameGset, pathwaySource=collection, location=loc)
...
ranks = difs$INNER
names(ranks) = rownames(difs)
I_gamma = sort(unlist(gseaRes$leadingEdge))
plot.ES(rank.statistic = ranks, gsea=gseaRes, gene.set=gset, name=nameGset, pathwaySource=collection, location=loc)
...
ranks = difs$OUTER
names(ranks) = rownames(difs)
O_gamma = sort(unlist(gseaRes$leadingEdge))
plot.ES(rank.statistic = ranks, gsea=gseaRes, gene.set=gset, name=nameGset, pathwaySource=collection, location=loc)
...
alpha = intersect(gset$HALLMARK_INTERFERON_ALPHA_RESPONSE,rownames(Data@data))
gamma = intersect(gset$HALLMARK_INTERFERON_GAMMA_RESPONSE, rownames(Data@data))
S_inter = union(S_alpha,S_gamma)
I_inter = union(I_alpha,I_gamma)
O_inter = union(O_alpha,O_gamma)
inter = unique(c(I_inter,O_inter,S_inter))
mat = Data@data[inter,]
...
sdat = t(apply(sdat,2,function(x) ifelse(x>=threshold, threshold, x)))
rownames(sdat) = rep("",nrow(sdat))
rownames(sdat)[col.ord][1000] = 'INNER'
p=levelplot(sdat[col.ord, row.ord], col.regions=colo, at=zak,  
            aspect = asp, xlab='', ylab='',
            scales = list(x = list(rot = 0, cex=3, lwd=0), tck=c(0,0), alternating=2, y = list(cex=0.9)), 
            colorkey = FALSE,
...
sdat = t(apply(sdat,2,function(x) ifelse(x>=threshold, threshold, x)))
rownames(sdat) = rep("",nrow(sdat))
rownames(sdat)[col.ord][1000] = 'OUTER'
p=levelplot(sdat[col.ord, row.ord], col.regions=colo, at=zak,  
            aspect = asp, xlab='', ylab='',
            scales = list(x = list(rot = 0, cex=3, lwd=0), tck=c(0,0), alternating=2, y = list(cex=0.9)), 
            colorkey = FALSE,
...
# plot heatmaps
grid.arrange(p.diff, p_C, p_I, p_O, p_S, ncol=5, widths=c(0.8,1,1,1,1))
```
...
i=ifelse(is.element(rownames(tab),I_alpha), 'I','')
o=ifelse(is.element(rownames(tab),O_alpha), 'O','')
s=ifelse(is.element(rownames(tab),S_alpha), 'S','')
lea = paste(i,o,s, sep='')
i=ifelse(is.element(rownames(tab),I_gamma), 'I','')
o=ifelse(is.element(rownames(tab),O_gamma), 'O','')
s=ifelse(is.element(rownames(tab),S_gamma), 'S','')
leg = paste(i,o,s, sep='')
le = ifelse(!(lea=="" & leg==""), 'a/g', '')
le = ifelse(lea !='' & leg=='', 'a',le)
le = ifelse(lea =='' & leg!='', 'g', le)
pct=t(apply(Data@raw.data,1, tapply, Data@ident, function(x) sum(x>0)/length(x)))
ipct=signif(pct[rownames(tab),]*100,2)
tab = data.frame(No = 1:nrow(tab), rownames(tab), tab, le, lea, leg, ipct)
names(tab) = c('No', 'Gene', 'logFC_C','logFC_I','logFC_O','logFC_S',
               'logAve_C','logAve_I','logAve_O','logAve_S','logAve_Tot',
               'LE', 'LE_alpha','LE_gamma', 'pct_C','pct_I','pct_O', 'pct_S')
...
dat=as.matrix(Data@data[all.features,])
