...
sopat=readRDS("data/SO_GSE146026_10X.rds")
...
sopat <- MapQuery(anchorset = anchors, reference = iso, query = sopat,
                  refdata = list(clusters = "DA_regions_layer"), reduction.model = "umap")
...
tso10X = sopat
tso10X_scores = prediction

sel.good = sopat$prediction.score.max > 0.5 & sopat$prediction.maxdiff>=0.25
good10X = subset(sopat, cells=sopat$Cell_ID[sel.good])

isoSEEP_ref=iso

save(tso10X, tso10X_scores, good10X, isoSEEP_ref, good10X,  file="data/10Xtrans_Regions.rda")
