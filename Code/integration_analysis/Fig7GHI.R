...
ss2_violin = VlnPlot(object=ss2, pt.size=0.1, log=T, slot='data', sort=T, features=x$Gene, split.by="Regions") +
  ...
  xlab("") +
  geom_boxplot(alpha=0.5, outlier.shape = NA, width=0.3, col='black', show.legend = FALSE) +
  stat_summary(fun = "mean", geom = "crossbar", width = 0.5, colour = "black", show.legend = FALSE) +   ylab("TNFAIP2 Log Expression")  +
  theme(plot.title = element_text(size = 12, face = 'plain')) +
  scale_fill_manual(values=colo2)+
  #stat_summary(fun=qua,geom='point', shape=21, stroke=1, show.legend = F, size=3) +
  theme(legend.position = 'none') +
...
tiff("Figures/FigGHI_11.20.22.tiff", width=12, height=5, compression = "lzw", res=300, units='in')
  print(

    (seep_violin | x10_violin | ss2_violin)  #+ plot_annotation(tag_levels = list(c("G","H","I"))) & theme(plot.tag = element_text(size = 16, face='plain'))
  )
dev.off()

pdf("Figures/FigGHI_11.20.22.pdf", width=12, height=5)
print(
