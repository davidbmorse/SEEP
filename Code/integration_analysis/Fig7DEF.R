...
paths = intersect(oras,rownames(iso))
...
heatmap = heatmap + theme(axis.text.y = element_text(size=8, colour = rep(rev(colo2), rev(table(paths_region[selpath]))))) + scale_color_manual(values = colo2) +
  ...
  scale_y_discrete(labels = rev(paths_names[selpath])) + geom_hline(yintercept = cumsum(rev(table(paths_region[selpath])))+0.5, color='white', size=0.3) + ggtitle("HGSOC Model Cells") +  theme(plot.title = element_text(size = 12, face = "plain"))
seep = heatmap
...
ss2_top_cells = intersect(top1, goodSS2_cells)
...
heatmap = heatmap + theme(axis.text.y = element_text(size=8, colour = rep(rev(colo2), rev(table(paths_region[selpath]))))) + scale_color_manual(values = colo2) +
  theme(legend.key.height = unit(1, 'cm'),
        ...
        legend.title=element_text(size=10)) +
  guides(colour = guide_legend(override.aes = list(size=3, shape=15, alpha = 1), title=NULL)) +
  scale_y_discrete(labels = rev(paths_names[selpath])) + geom_hline(yintercept = cumsum(rev(table(paths_region[selpath])))+0.5, color='white', size=0.3) + ggtitle("HGSOC Ascites Tumor Cells Isolated") +  theme(plot.title = element_text(size = 12, face = "plain"))
...
x10_top_cells = intersect(top1, good10X_cells)
...
heatmap = heatmap + theme(axis.text.y = element_text(size=8, colour = rep(rev(colo2), rev(table(paths_region[selpath]))))) + scale_color_manual(values = colo2) +
  theme(legend.key.height = unit(1, 'cm'),
        ...
        legend.title=element_text(size=10)) +
        guides(colour = guide_legend(override.aes = list(size=3, shape=15, alpha = 1), title=NULL)) +
        scale_y_discrete(labels = rev(paths_names[selpath])) + geom_hline(yintercept = cumsum(rev(table(paths_region[selpath])))+0.5, color='white', size=0.3) +
...
tiff(sprintf("Figures/Fig7DEF_11.20.22.tiff"), compress="lzw", res=300, units='in', width=14, height=7)
Fig7DEF = (seep/x10/ss2) + plot_layout(guides = "collect")
print(Fig7DEF)
dev.off()

pdf(sprintf("Figures/Fig7DEF_11.20.22.pdf"), width=14, height=7)
Fig7DEF = (seep/x10/ss2) + plot_layout(guides = "collect")
#+ plot_annotation(tag_levels = list(c("D", "E", "F"))) & theme(plot.tag = element_text(size = 20))
print(Fig7DEF)
dev.off()
...
for(i in names(lcells)){
  ...
  tiff(sprintf("Figures/Additional/10X_PathwaysHeatmapTop_%s.tiff", i), compress="lzw", res=300, units='in', width=16, height=2.5)
...
save(lora, file="data/top_pathways.rda")
save(seep_top_cells, ss2_top_cells, x10_top_cells, file="data/top_cells.rda")
